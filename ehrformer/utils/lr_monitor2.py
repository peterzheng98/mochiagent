...
class LearningRateMonitor(Callback):
    ...
    def __init__(self, logging_interval: Optional[str] = None, log_momentum: bool = False, logger_indexes=None) -> None:
        if logging_interval not in (None, "step", "epoch"):
            raise MisconfigurationException("logging_interval should be `step` or `epoch` or `None`.")

        self.logging_interval = logging_interval
        self.log_momentum = log_momentum
        self.logger_indexes = logger_indexes
        if isinstance(self.logger_indexes, int):
            self.logger_indexes = [self.logger_indexes]
    ...
    def on_train_batch_start(self, trainer: "pl.Trainer", *args: Any, **kwargs: Any) -> None:
        ...
        if self.logging_interval != "epoch":
            ...
            if latest_stat:
                if self.logger_indexes is None:
                    for logger in trainer.loggers:
                        logger.log_metrics(latest_stat, step=trainer.fit_loop.epoch_loop._batches_that_stepped)
                else:
                    for index in self.logger_indexes:
                        trainer.loggers[index].log_metrics(latest_stat, step=trainer.fit_loop.epoch_loop._batches_that_stepped)
