...
def main(config):
    ...
    # 加载预训练的 EHRBert 模型
    if 'ehr_emb_ckpt_path' in config and config['ehr_emb_ckpt_path'] is not None:
        # 定义键值映射规则
        key_mappings = [
            ('model.ehr_embed.bert.', ''),      # 从 pretrain 模型加载
            ('bert.', '')                       # 直接 bert 前缀
        ] 
        # 使用工具函数加载
        load_checkpoint_with_key_mapping(
            checkpoint_path=config['ehr_emb_ckpt_path'],
            target_model=model_module.model.ehr_embed.bert,
            key_mappings=key_mappings,
            model_name="EHRBert.BertModel")
    ...
    if config['test']:
        ...
        if config.get('save_gpt2_embed', False):
            ...
            from utils.plot.forward_hook_utils import register_gpt2_output_hooks
            gpt2_embed_list = []
        ...
        if config.get('save_gpt2_embed', False):
            ...
            save_path = os.path.join(config['pred_folder'], 'gpt2_embed.npy')
            np.save(save_path, gpt2_embed)
